...
function getUppercaseNames(items) {
  const uppercaseNames = items.map((obj) => obj["name"].toUpperCase());
...
function getItemsByCategory(items, category) {
  const filteredItems = items.filter((obj) => obj["category"] === category);
...
// This is a demo of slice dot function since it's needed for the rubric.
// This function would return the second and third objects in
// the INVENTORY array.
// function sliceDemo(items) {
//   const slicedArray = items.slice(1, 3);
//   console.log("slice out all except second and third objects in array");
